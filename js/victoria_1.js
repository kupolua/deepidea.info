   ...
   'use strict';
	...
	$(document).scroll(function() { 
		...
		if($('section.home').length && $(this).scrollTop() < 180) {
			$('.header .navigation li').stop().animate({ opacity: '1' });
                        $('.header').stop().animate({ backgroundColor: 'rgba(17,17,17,0)' });
		} else {
			$('.header .navigation li').stop().animate({ opacity: '0.5' });
                        $('.header').stop().animate({ backgroundColor: 'rgba(17,17,17,0.75)' });
		}
...
$(window).load(function() {
	...
	//Animations
	setTimeout(function(){$('.header').addClass('animated fadeInDown')},100);
	setTimeout(function(){$('#home .content').addClass('animated fadeInDown')},400);
